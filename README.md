...
The `SPARTNMessage` class does not currently perform a full decode of SPARTN protocol messages or decrypt an encrypted payload; it decodes just enough information to identify message type/subtype, payload length and other key metadata. Full decode will be added in due course as and when voluntary development time permits - contributions welcome!
